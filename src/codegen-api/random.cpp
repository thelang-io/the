...
const std::vector<std::string> codegenRandom = {
  R"(double random_randomFloat (unsigned char o1, double n1, unsigned char o2, double n2, int line, int col) {)" EOL
  R"(  double a = o1 == 0 ? 0 : n1;)" EOL
  R"(  double b = o2 == 0 ? 1 : n2;)" EOL
  R"(  unsigned char d[4];)" EOL
  R"(  if (_{RAND_bytes}(d, 4) != 1) {)" EOL
  R"(    _{error_assign}(&_{err_state}, _{TYPE_error_Error}, (void *) _{error_Error_alloc}(_{str_alloc}("failed to generate random bytes"), (_{struct str}) {_{NULL}, 0}), (void (*) (void *)) &_{error_Error_free}, line, col);)" EOL
  ...
  R"(_{int32_t} random_randomInt (unsigned char o1, _{int32_t} n1, unsigned char o2, _{int32_t} n2, int line, int col) {)" EOL
  ...
  R"(_{struct str} random_randomStr (unsigned char o1, _{struct str} n1, unsigned char o2, _{int32_t} n2, int line, int col) {)" EOL
