...
class TypeMap {
 public:
  ...
  Type *createFn (const std::vector<TypeFnParam> &, Type *, bool, bool, const std::optional<TypeCallInfo> & = std::nullopt);
  Type *createMap (Type *, Type *);
  Type *createMethod (const std::vector<TypeFnParam> &, Type *, bool, bool, TypeCallInfo);
