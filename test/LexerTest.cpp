...
TEST(LexerTest, ThrowsOnUnknownToken) {
  ...
  }, "Error: tried stringify unknown token");
}
...
TEST(LexerTest, LexKeywordsWhitespace) {
  auto reader = testing::NiceMock<MockReader>(
    " break catch continue elif else enum false fn if is loop main mut nil obj return throw true try union "
  );
  ...
  EXPECT_EQ(std::get<1>(lexer.next()).str(), "KW_RETURN(1:75-1:81): return");
  EXPECT_EQ(std::get<1>(lexer.next()).str(), "KW_THROW(1:82-1:87): throw");
  EXPECT_EQ(std::get<1>(lexer.next()).str(), "KW_TRUE(1:88-1:92): true");
  EXPECT_EQ(std::get<1>(lexer.next()).str(), "KW_TRY(1:93-1:96): try");
  EXPECT_EQ(std::get<1>(lexer.next()).str(), "KW_UNION(1:97-1:102): union");
  EXPECT_EQ(std::get<1>(lexer.next()).str(), "EOF(1:103-1:103)");
}
...
TEST(LexerTest, LexLitStr) {
  ...
  EXPECT_EQ(std::get<1>(lexer.next()).str(), R"(LIT_STR(2:58-2:81): "\\\\0\\\\r\\\\t\\"\\'\\\\\\0\\r\\t")");
}
