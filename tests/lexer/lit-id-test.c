...
TEST(lexer_lit_id, is_char_and_is_char_start) {
  ASSERT_TRUE(lexer_lit_id_is_char('A'));
  ASSERT_TRUE(lexer_lit_id_is_char('a'));
  ASSERT_TRUE(lexer_lit_id_is_char('0'));
  ASSERT_TRUE(lexer_lit_id_is_char('9'));
  ASSERT_TRUE(lexer_lit_id_is_char('_'));
  ASSERT_FALSE(lexer_lit_id_is_char('@'));

  ASSERT_TRUE(lexer_lit_id_is_char_start('A'));
  ASSERT_TRUE(lexer_lit_id_is_char_start('a'));
  ASSERT_TRUE(lexer_lit_id_is_char_start('_'));
  ASSERT_FALSE(lexer_lit_id_is_char_start('0'));
  ASSERT_FALSE(lexer_lit_id_is_char_start('9'));
  ASSERT_FALSE(lexer_lit_id_is_char_start('@'));
}

int main () {
  TEST_RUN(lex_lit_id, works);
  TEST_RUN(lexer_lit_id, is_char_and_is_char_start);
}
